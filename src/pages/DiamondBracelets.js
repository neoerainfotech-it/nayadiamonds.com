...
// Use bracelet_043.jpg to bracelet_048.jpg for Diamond Bracelets (6 products)
const diamondBraceletImages = [
  '/images/bracelet_043.jpg',
  '/images/bracelet_044.jpg',
  '/images/bracelet_045.jpg',
  '/images/bracelet_046.jpg',
  '/images/bracelet_047.jpg',
  '/images/bracelet_048.jpg',
];

const products = diamondBraceletImages.map((img, idx) => ({
  id: idx + 1,
  name: `Diamond Bracelet ${idx + 1}`,
  price: 18000 + idx * 1500,
  image: img,
  description: "Dazzling diamond bracelet with premium finish.",
}));
...
export default function DiamondBracelets() {
  ...
  return (
    <div className="container mx-auto py-10 px-4">
      <h1 className="text-3xl font-bold mb-6 text-yellow-900">Diamond Bracelets</h1>
          ...
          <div key={product.id} className="group bg-white rounded-xl shadow-md flex flex-col items-center hover:shadow-xl transition-shadow p-4 relative overflow-hidden">
            <div className="relative w-32 h-32 mb-3">
              ...
              <div className="shop-action flex flex-col gap-2 absolute top-1/2 right-2 -translate-y-1/2 opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-10">
                ...
                {/* Quick View */}
                <button className="woosq-btn bg-white/90 p-2 rounded-full shadow hover:bg-blue-100 flex items-center justify-center" aria-label="Quick View" onClick={e => { e.stopPropagation(); setQuickView(product); }}>
            ...
            <div className="text-sm text-gray-500 mb-2 text-center">Diamond Bracelet</div>
            {/* Order Now Button below price */}
            <button className="w-full px-4 py-2 bg-green-500 text-white rounded-lg font-semibold hover:bg-green-600 transition text-center mt-1">Make Your Order</button>
          </div>
        ))}
      </div>
      {/* Quick View Modal */}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl shadow-2xl p-8 max-w-md w-full relative">
            ...
            <div className="flex gap-2 justify-center mb-2">
              <button className="px-4 py-2 bg-yellow-500 text-white rounded-lg font-semibold hover:bg-yellow-600 transition" onClick={() => setQuickView(null)}>
                Add to Cart
              </button>
              <button className="px-4 py-2 bg-green-500 text-white rounded-lg font-semibold hover:bg-green-600 transition" onClick={() => setQuickView(null)}>
                Make Your Order
              </button>
            </div>
          </div>
