...
const diamondNecklaceImages = Array.from({ length: 15 }, (_, i) => `/images/necklace_${115 + i}.jpg`);

const products = diamondNecklaceImages.map((img, idx) => ({
  id: idx + 1,
  name: `Diamond Necklace ${idx + 1}`,
  price: 25000 + idx * 2000,
  image: img,
  description: "Dazzling diamond necklace with premium finish.",
}));
...
export default function DiamondNecklaces() {
  ...
  return (
    <div className="container mx-auto py-10 px-4">
      <h1 className="text-3xl font-bold mb-6 text-yellow-900">Diamond Necklaces</h1>
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6">
        {products.map((product) => (
          <div key={product.id} className="group bg-white rounded-xl shadow-md flex flex-col items-center hover:shadow-xl transition-shadow p-4 relative overflow-hidden">
            ...
            <div className="text-sm text-gray-500 mb-2 text-center">Diamond Necklace</div>
