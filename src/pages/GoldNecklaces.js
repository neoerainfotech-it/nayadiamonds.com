...
const goldNecklaceImages = Array.from({ length: 15 }, (_, i) => `/images/necklace_${100 + i}.jpg`);

const products = goldNecklaceImages.map((img, idx) => ({
  id: idx + 1,
  name: `Gold Necklace ${idx + 1}`,
...
export default function GoldNecklaces() {
  ...
  return (
    <div className="container mx-auto py-10 px-4">
      <h1 className="text-3xl font-bold mb-6 text-yellow-900">Gold Necklaces</h1>
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6">
        {products.map((product) => (
          <div key={product.id} className="group bg-white rounded-xl shadow-md flex flex-col items-center hover:shadow-xl transition-shadow p-4 relative overflow-hidden">
            ...
            <div className="text-sm text-gray-500 mb-2 text-center">Gold Necklace</div>
