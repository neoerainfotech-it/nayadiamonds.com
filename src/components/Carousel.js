...
const slideData = [
  ...
  },
  {
    ...
    background: "url('/images/ddhome.jpeg') center/cover no-repeat",
  },
  {
    id: 3,
...
function Carousel() {
  ...
  return (
    <div style={{ position: 'relative', overflow: 'hidden', height: '100vh', width: '100%' }}>
        ...
        >
          ...
          >
            <h2 className="slide-title" style={{
              fontSize: '3rem', fontWeight: 600, marginBottom: '1rem', textTransform: 'uppercase', letterSpacing: '2px', color: '#fff',
            ...
            <p className="slide-description" style={{
              fontSize: '1.25rem', fontWeight: 300, marginBottom: '2rem', maxWidth: 600, color: '#fff',
              transform: index === currentSlide ? 'translateY(0)' : 'translateY(50px)',
              opacity: index === currentSlide ? 1 : 0,
              transition: 'transform 1s ease-out, opacity 1s ease-out',
            ...
            <a
              ...
              style={{
                padding: '0.75rem 2rem', backgroundColor: '#fff', color: '#000', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '1px', border: 'none', borderRadius: '6px', cursor: 'pointer',
                transform: index === currentSlide ? 'translateY(0)' : 'translateY(50px)',
                opacity: index === currentSlide ? 1 : 0,
      ...
      {/* Dots */}
      <div style={{ position: 'absolute', bottom: '2rem', right: '6rem', display: 'flex', gap: '1rem', zIndex: 10 }}>
        {slideData.map((_, index) => (
          <div
            key={index}
            className={`carousel-dot${index === currentSlide ? ' active' : ''}`}
            style={{
              width: 12, height: 12, borderRadius: '50%', backgroundColor: index === currentSlide ? '#fff' : 'rgba(255,255,255,0.5)', cursor: 'pointer', transition: 'background-color 0.3s, transform 0.3s', transform: index === currentSlide ? 'scale(1.2)' : 'scale(1)',
            }}
      ...
      {/* Arrows */}
      <div style={{ position: 'absolute', top: '50%', width: '100%', display: 'flex', justifyContent: 'space-between', padding: '0 2rem', transform: 'translateY(-50%)', zIndex: 10 }}>
        <div
          className="carousel-arrow prev"
          style={{ width: 50, height: 50, borderRadius: '50%', backgroundColor: 'rgba(255,255,255,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', transition: 'background-color 0.3s' }}
